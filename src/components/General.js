  ...
  render() {
    ...
    const display = <div className="d-flex flex-column align-items-center display mt-4">
      <div className="container">
        <h4 className="mx-auto my-4 text-center">General Information</h4>
        <h5><span>{name}</span></h5>    
        {this.state.country && <CountrySelect className="mb-2" id="country" value={this.state.country} disabled/> } 
        <h5 className="mb-4 mt-4"><FontAwesomeIcon color="green" className="mr-4 ml-1" size="lg" icon={faMobileAlt} /> <a href={"tel:"+phone}>{phone}</a></h5>  
        <hr></hr>
        <h5 className="mb-4"><FontAwesomeIcon color="orange" className="mr-3" size="lg" icon={faEnvelope} />  <a href={"mailto:" + email}>{email}</a></h5>  
        <hr></hr>
        {this.state.git && <h5 className="mb-2"><FontAwesomeIcon color="indigo" className="mr-4" size="lg" icon={faGithubAlt} /><a href={git}>Github</a></h5> }  
      </div>
      
      <button className="btn btn-outline-success mt-4 p-2 w-50" type="button" onClick={this.onEdit}>Edit</button>
    </div>;
    ...
    return (
      ...
      <form className="px-lg-5 px-md-3" onSubmit={this.onSubmit} id="general-form">
        ...
        <input placeholder="Your first and last name" required className="mb-2" value={this.state.name}  onChange={this.onChange} id="name" name="name"></input>
